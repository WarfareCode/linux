...
#ifndef __BW_AxisAlignedBox_H_
#define __BW_AxisAlignedBox_H_
...
// 作者：皮学贤 
// 日期：2010-06-07 
// 功能：正立方体（立方体的所有棱都沿坐标轴平行） From OGRE?
//================================================================================
	...
	*/
	...
	public:
		...
		*/
		...
		inline AABox(Extent e) : mpCorners(0)
		{
			setMinimum( -0.5, -0.5, -0.5 );
			setMaximum( 0.5, 0.5, 0.5 );
			mExtent = e;
		...
		inline AABox(const AABox & rkBox) : mpCorners(0)
		{
			if (rkBox.isNull())
				setNull();
		...
		inline AABox( const Vec3& min, const Vec3& max ) : mpCorners(0)
		{
			setExtents( min, max );
		}

		inline AABox( Real mx, Real my, Real mz,Real Mx, Real My, Real Mz ) : mpCorners(0)
		{
		...
		*/
		inline const Vec3& getMinimum(void) const
		{ 
			return mMinimum; 
		}
		...
		*/
		inline const Vec3& getMaximum(void) const
		{ 
			return mMaximum;
		}
		...
		*/
		inline const Vec3* getAllCorners(void) const
		{
		...
		*/
		Vec3 getCorner(CornerEnum cornerToGet) const
		{
			switch(cornerToGet)
			{
			...
			case FAR_LEFT_TOP:
				return Vec3(mMinimum.x, mMaximum.y, mMinimum.z);

			case FAR_RIGHT_TOP:
				return Vec3(mMaximum.x, mMaximum.y, mMinimum.z);

			case FAR_RIGHT_BOTTOM:
				return Vec3(mMaximum.x, mMinimum.y, mMinimum.z);

			case NEAR_RIGHT_BOTTOM:
				return Vec3(mMaximum.x, mMinimum.y, mMaximum.z);

			case NEAR_LEFT_BOTTOM:
				return Vec3(mMinimum.x, mMinimum.y, mMaximum.z);

			case NEAR_LEFT_TOP:
				return Vec3(mMinimum.x, mMaximum.y, mMaximum.z);
		...
		////BW_CAL_API friend std::ostream& operator<<( std::ostream& o, const AABox aab )
		////{
		////	switch (aab.mExtent)
		////	{
		////	case EXTENT_NULL:
		...
		*/
		void transformAffine(const Matrix4& m)
		{
			...
			Vec3 centre = getCenter();
			Vec3 halfSize = getHalfSize();

			Vec3 newCentre = m.transformAffine(centre);
			Vec3 newHalfSize(
		...
		*/
		...
		inline bool intersects(const AABox& b2) const
		{
		...
		inline AABox intersection(const AABox& b2) const
		{
            if (this->isNull() || b2.isNull())
			{
				return AABox();
			...
			Vec3 intMin = mMinimum;
            Vec3 intMax = mMaximum;
        ...
        */
		...
		// special values
		static const AABox BOX_NULL;
		static const AABox BOX_INFINITE;
	};
